...
import { OptionCombobox } from "@/components/common/OptionCombobox";
    ...
    if (!data.salaryNegotiable) {
      ...
      if (!hasMin && !hasMax) {
        ctx.addIssue({
          ...
          path: ["salaryMin"],
          message: 'Vui lòng nhập mức lương min/max hoặc bật "Thỏa thuận"',
        });
      }
      if (
        hasMin &&
...
export function JobModal({ open, onOpenChange, job }: JobModalProps) {
  ...
  const onSubmit = async (values: JobFormValues) => {
    ...
    const yoeMin =
      typeof values.yoeMin === "number" ? values.yoeMin : undefined;
    const yoeMax =
      typeof values.yoeMax === "number" ? values.yoeMax : undefined;
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Nhóm ngành <span className="text-destructive">*</span>
                    </FormLabel>
                    <Select
                      value={field.value}
                      onValueChange={(v) => {
