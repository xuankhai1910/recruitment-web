...
function ExpiredJobCardWrapper({ item }: { item: SavedJob }) {
          ...
          <p className="text-slate-500">
            {appliedKeyword ? (
              <>
                ...
                quả cho "<span className="text-blue-600">{appliedKeyword}</span>
                "
              </>
            ) : (
              <>
                <span className="font-medium text-slate-900">{total}</span> việc
                làm trong danh sách
              </>
            )}
          </p>
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {items.map((item) => {
            ...
            if (!item.job) return null;
            return <JobCard key={item._id} job={item.job} variant="row" />;
          })}
        </div>
      )}
