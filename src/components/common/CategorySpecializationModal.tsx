			...
			>
					...
					<div className="relative min-h-0 overflow-y-auto bg-white" ref={scrollRef}>
						...
						<div className="px-6 pb-6 pt-2">
							...
							{filteredCategories.length === 0 && (
								<div className="px-2 py-8 text-center text-[13px] text-slate-600">
									Không tìm thấy kết quả cho "<b>{search}</b>". Thử từ khóa khác.
								</div>
							)}
						</div>
					</div>
				...
				<footer className="flex items-center justify-between gap-3 border-t border-line bg-white px-6 py-3.5">
					...
					<div className="flex gap-2.5">
						<button className={ui_btnOutline} onClick={() => onOpenChange(false)}>
							Hủy
						</button>
						<button
							className={ui_btnAccent}
							onClick={() => onConfirm({ specializations: selected })}
						>
							Chọn
							{totalSelected > 0 && (
