...
function ActivePill({ label, onRemove }: { label: string; onRemove: () => void }) {
	return (
		<span className="inline-flex items-center gap-1.5 rounded-full bg-teal-50 px-2.5 py-1 text-xs font-semibold text-teal-700">
			{label}
			<button onClick={onRemove} aria-label={`Bỏ ${label}`} className="opacity-70 hover:opacity-100">
				<X className="h-3 w-3" />
			</button>
		</span>
	);
...
export function JobsPage() {
	...
	if (levelParam) {
		...
		queryParams.level = levels.length > 1 ? `/${levels.join("|")}/i` : levels[0];
	}
			...
			<div className="mb-6 flex flex-wrap items-end justify-between gap-6">
				<div>
					...
					<p className="mt-3 text-[15px] text-slate-600">
						{meta ? (
							<>
								Có <b className="font-display font-bold text-ink">{meta.total}</b>{" "}
								việc làm phù hợp
								{keywordParam ? (
									<>
										{" "}
										cho "<b className="font-display font-bold text-ink">{keywordParam}</b>"
									</>
								) : null}
							</>
						) : (
			...
			<div className="grid grid-cols-1 gap-8 lg:grid-cols-[280px_1fr]">
				...
				<aside className="rounded-xl border border-line bg-white p-6 lg:sticky lg:top-24 lg:self-start">
					<div className="mb-5 flex items-center justify-between">
						<h3 className="font-display text-base font-bold text-ink">Bộ lọc</h3>
						{activeFilterCount > 0 && (
							<button
					...
					<div>
						...
						{SALARY_RANGES.filter((r) => r.key !== "all").map((r) => (
							<CheckRow
								...
								onClick={() =>
									setSingleParam("salary", salaryParam === r.key ? "all" : r.key)
								}
							/>
						))}
					</div>
				...
				<div>
					{activeFilterCount > 0 && (
						<div className="mb-5 flex flex-wrap gap-2">
							{keywordParam && (
								<ActivePill label={`"${keywordParam}"`} onRemove={() => applySearch("")} />
							)}
							...
							{selectedLevels.map((l) => (
								<ActivePill key={"lv-" + l} label={l} onRemove={() => setMultiParam("level", l)} />
							))}
							{locations.map((loc) => (
								<ActivePill
									key={"loc-" + loc}
									label={loc}
									onRemove={() => applyLocations(locations.filter((x) => x !== loc))}
								/>
							))}
							{workModeParam && (
								<ActivePill label={workModeParam} onRemove={() => setSingleParam("workMode", "")} />
							)}
							{jobTypeParam && (
								<ActivePill label={jobTypeParam} onRemove={() => setSingleParam("jobType", "")} />
							)}
							{salaryParam !== "all" && (
								<ActivePill
									label={SALARY_RANGES.find((r) => r.key === salaryParam)?.label ?? ""}
					...
					{isLoading ? (
						...
						>
							{SKELETON_ROWS.map((r) => (
								<Skeleton
									key={r}
									className={view === "list" ? "h-24 rounded-xl" : "h-60 rounded-xl"}
								/>
							))}
						</div>
					) : jobs.length === 0 ? (
						<div className={ui.empty}>
							...
							{activeFilterCount > 0 && (
								<button className={cn(ui.btnOutline, "mt-5")} onClick={resetFilters}>
									Xóa bộ lọc
								</button>
							)}
						</div>
