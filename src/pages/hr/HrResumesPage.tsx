...
export function HrResumesPage() {
  ...
  const [sortField, setSortField] = useState<
    "email" | "userId.name" | "match.score" | null
  >(null);
  const [sortDir, setSortDir] = useState<"asc" | "desc">("asc");

  const quota = useMatchBatchQuota();
  const batch = useBatchAnalyzeResumes();
  const jobsQuery = useJobsByAdmin({
    current: 1,
    pageSize: 1000,
    sort: "name",
  });

  const handleSort = (field: "email" | "userId.name" | "match.score") => {
    if (sortField !== field) {
      setSortField(field);
      setSortDir("asc");
    } else if (sortDir === "asc") {
  ...
  const SortIcon = ({
    ...
    field: "email" | "userId.name" | "match.score";
  }) => {
    if (sortField !== field)
  ...
  const getJobId = (r: Resume) => {
    if (!r.jobId) return undefined;
    return typeof r.jobId === "object" ? r.jobId._id : r.jobId;
  };
  ...
  const columns: Column<Resume>[] = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        if (!id) return <span className="block truncate">{name}</span>;
        return (
          <Link
            to={`/jobs/${id}`}
            className="block truncate font-medium text-primary hover:underline"
    ...
    },
    {
      ...
      labelNode: (
        <button
          type="button"
          className="flex cursor-pointer items-center gap-1 transition-colors hover:text-foreground"
          onClick={() => handleSort("match.score")}
        >
          Độ phù hợp
          <SortIcon field="match.score" />
        </button>
      ),
      render: (row) => {
        if (!row.match) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-col items-end gap-1">
          <Button
            onClick={() => batch.run()}
            disabled={batch.running || quota.data?.remaining === 0}
            className="cursor-pointer bg-teal-600 text-white transition-colors duration-150 hover:bg-teal-700"
            title="Chấm độ phù hợp cho các CV chưa được phân tích"
          >
            {batch.running ? (
            ...
            {batch.running
              ? `Đang phân tích ${batch.progress.done}/${batch.progress.total}…`
              : "Phân tích tất cả CV chưa chấm"}
          </Button>
          {quota.data && (
            <span className="text-xs text-muted-foreground">
              Còn {quota.data.remaining}/{quota.data.limit} lượt phân tích trong
              tháng
            </span>
          )}
        </div>
          ...
          <Popover open={jobFilterOpen} onOpenChange={setJobFilterOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                variant="outline"
                role="combobox"
                aria-expanded={jobFilterOpen}
                className={cn(
                  "h-9 w-full justify-between font-normal sm:w-[300px]",
                  !selectedJob && "text-muted-foreground",
                )}
              >
                <span className="truncate">
                  {selectedJob ? selectedJob.name : "Tất cả công việc"}
                </span>
            ...
            >
              <Command
                filter={(_value, searchValue, keywords) => {
                  const haystack = nonAccentVietnamese(
                    keywords?.[0] ?? "",
                  ).toLowerCase();
                  const needle = nonAccentVietnamese(searchValue).toLowerCase();
                  return haystack.includes(needle) ? 1 : 0;
                }}
              >
                <CommandInput placeholder="Tìm công việc..." />
                <CommandList className="max-h-64 overflow-y-auto ![scrollbar-width:thin] [&::-webkit-scrollbar]:!block [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border">
                  <CommandEmpty>Không tìm thấy công việc</CommandEmpty>
                  <CommandItem
                    value="ALL"
                    keywords={["Tất cả công việc", "tat ca cong viec", "all"]}
                    onSelect={() => {
                      setJobId("ALL");
                      setPage(1);
                      setJobFilterOpen(false);
                    }}
                    data-checked={jobId === "ALL"}
                  >
                    <X className="size-3.5 text-muted-foreground" />
                    <span className="truncate">Tất cả công việc</span>
                  </CommandItem>
                  {jobs.map((job) => (
                    <CommandItem
                      key={job._id}
